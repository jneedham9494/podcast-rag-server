...
def extract_guest_from_description(description, podcast_name):
    ...
    # Generic patterns (less reliable, use as fallback)
    patterns = [
        r'(?:guest|Guest):\s+([A-Z][a-zA-Z\'\-\.]+(?:\s+[A-Z][a-zA-Z\'\-\.]+){1,3})',
        r'(?:talks with|speaks with|joined by|interview with|conversation with)\s+([A-Z][a-zA-Z\'\-\.]+(?:\s+[A-Z][a-zA-Z\'\-\.]+){1,3})',
        r'^([A-Z][a-zA-Z\'\-\.]+(?:\s+[A-Z][a-zA-Z\'\-\.]+){1,3})\s+(?:joins|discusses|talks about)',
    ]
