#!/usr/bin/env python3
"""
Guest Extractor - FINAL VERSION
Fixes: lowercase particles (van, de, von), Adam Friedland matching, Odd Lots "with" pattern
"""
...
def extract_guest_from_title(title, podcast_name):
    ...
    # Adam Friedland Show: "GUEST NAME Talks Topic" (FIXED - better matching)
    ...
    if 'odd lots' in podcast_lower:
        # Pattern 1: "Guest Name on Topic" (IMPROVED - handles particles like "van", "de")
        match = re.match(r'^([A-Z][a-zA-Z\'\-\.]+(?:\s+(?:van|de|von|di|da|del|della|le|la)?[A-Z]?[a-zA-Z\'\-\.]+)+)\s+on\s+', title_clean, re.IGNORECASE)
        if match:
            guest = match.group(1).strip()
            if not any(x in guest for x in ['Big Take', 'Everybody', 'The ', 'Why ', 'How ', 'Inside ', 'What ', 'Lots More']):
                return guest

        # Pattern 2: "Topic With Guest Name" or "...with Guest Name on..."
...
def extract_guests_from_metadata():
    ...
    for metadata_file in sorted(metadata_dir.glob("*.json")):
        ...
        try:
            ...
            extraction_rate = (extracted_count/episode_count*100) if episode_count > 0 else 0
            print(f"  Extracted {extracted_count} guests from {episode_count} episodes ({extraction_rate:.1f}%)")
