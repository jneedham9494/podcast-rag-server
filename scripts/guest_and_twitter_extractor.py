...
def extract_guest_from_description(description, podcast_name):
    ...
    # OPTIMIZED: Enhanced universal fallback patterns
    # These run for ALL podcasts after podcast-specific patterns fail
    # Based on learnings from cross-podcast pattern testing
    patterns = [
        ...
        # NEW: Proven patterns from Chapo/TRUE ANON (gained +105 real guests on Chapo)
        r"(?:We're|we're)\s+joined\s+(?:again\s+)?by\s+(?:[\w\s,\(\)-]+?)?([A-Z][\w\'\-\.]+(?:\s+[A-Z]?[\w\'\-\.]+){1,4})",
        ...
        r'(?:We|we)\s+(?:welcome|bring\s+back)\s+(?:[\w\s,\(\)-]+?)?([A-Z][\w\'\-\.]+(?:\s+[A-Z]?[\w\'\-\.]+){1,4})',
        r'(?:host|Host)\s+(?:talks|chats|speaks)\s+with\s+(?:[\w\s,\(\)-]+?)?([A-Z][\w\'\-\.]+(?:\s+[A-Z]?[\w\'\-\.]+){1,4})',
        r'This\s+(?:week|episode):\s+([A-Z][\w\'\-\.]+(?:\s+[A-Z]?[\w\'\-\.]+){1,4})',

        # Professional title patterns (work across many podcasts)
        r'(?:Author|Writer|Journalist|Reporter|Professor|Comedian|Director|Actor)\s+([A-Z][\w\'\-\.]+(?:\s+[A-Z]?[\w\'\-\.]+){1,4})\s+(?:joins|returns|stops by)',
    ]
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            # Require at least 2 words to reduce false positives
            if len(guest.split()) >= 2:
                return guest
...
def clean_guest_name(name):
    ...
    # NEW: Remove trailing prepositions and common patterns (from false positive analysis)
    ...
    # NEW: Remove job titles at the start
    name = re.sub(r'^(?:Author|Writer|Journalist|Professor|Director|Comedian|Reporter|Activist)\s+', '', name)

    # NEW: CORRECTED - Extract person name from "Company CEO Name" patterns
    ...
    # NEW: Remove Fed position titles (extract person name)
    ...
    # NEW: Clean "about her/his/their work" patterns
    ...
    skip_terms = ['podcast', 'show', 'episode', 'special', 'bonus', 'trailer', 'live',
                  ...
                  'economics and', 'about her', 'about his']  # Added from cleanup

    if any(term in name_lower for term in skip_terms):
        return None

    # Skip if starts with question words/articles
    if any(name_lower.startswith(q) for q in ['how ', 'why ', 'what ', 'the ', 'a ', 'an ', 'of ']):
        return None
